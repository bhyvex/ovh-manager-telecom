angular.module('managerApp').controller('TelecomTelephonyAliasConfigurationChangeTypeCtrl', class TelecomTelephonyAliasConfigurationChangeTypeCtrl {
  constructor(
    ...
    OvhApiTelephony, Toast, telephonyBulk,
    voipService, voipServiceAlias, voipServiceTask,
  ) {
    this.$q = $q;
    this.$state = $state;
  ...
  changeType() {
    ...
    this.voipServiceAlias.changeNumberFeatureType({
      ...
      this.Toast.success(this.$translate.instant('telephony_alias_change_type_ok'));
    ...
    return (bulkResult) => {
      ...
      checkServerTasksStatus(bulkResult.success).then(() => {
        ...
        if (this.successfulTasks.length < bulkResult.success.length) {
          this.Toast.warn([this.$translate.instant('telephony_alias_config_change_type_bulk_server_tasks_some_error')]);
        }
        ...
        this.Toast.error([this.$translate.instant('telephony_alias_config_change_type_bulk_server_tasks_all_error')]);
      }).finally(() => {
        this.loading = false;
      });
    };

angular.module('managerApp').controller('PackCtrl', class PackCtrl {
  constructor(
    ...
    DASHBOARD, PACK,
    OvhApiPackXdsl, resiliationNotification, SidebarMenu, Toast,
  ) {
    this.$q = $q;
    this.$scope = $scope;
  ...
  initFrames() {
    ...
    return this.getAllFrames(this.$stateParams.packName).catch((err) => {
      if (err.status !== 460 && err.status !== 403) {
        this.Toast.error([this.$translate.instant('pack_xdsl_oops_an_error_is_occured'), err.data ? err.data.message : ''].join(' '));
      }
      return this.$q.reject(err);
    }).finally(() => {
